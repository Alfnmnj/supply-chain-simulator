# app.py (Strategic Simulation Platform v12.1 - Final Verified)
    ...
    if st.session_state.mode == "Guided":
        ...
        with st.expander("4. Define Geopolitical Scenario", expanded=True):
            ...
            SCENARIO_PARAMS = {
                ...
                "Regional Conflict": {"tariff": 0.25, "supply_cut": 0.40, "delay": 21, "desc": "Models significant tariffs, supply cuts, and shipping delays."},
    ...
    if st.session_state.mode == "Guided":
        if secondary_supplier is None:
            ...
            with col_chart:
                ...
                df_plot = pd.DataFrame([baseline_results, resilient_results], index=['Baseline', 'Resilient']).T
                ...
                fig = px.bar(df_plot_melted, x="index", y="Value", color="Strategy", barmode="group",
            ...
            with col_gauge:
                st.markdown("<h6 style='text-align: center;'><i class='bi bi-exclamation-octagon-fill'></i> Stockout Risk Assessment</h6>", unsafe_allow_html=True)
            ...
            with st.expander("Show Actionable Recommendations & BCP", expanded=True):
                ...
                st.markdown(f"""<h4>Business Continuity Plan for: <b>{selected_component}</b></h4><p>Based on a simulated <strong>{scenario_choice}</strong> scenario targeting <strong>{primary_supplier['Country']}</strong>.</p>
                <h5><i class="bi bi-exclamation-triangle-fill" style="color: #ff4b4b;"></i>  Threat Assessment (Baseline Single-Source Strategy)</h5>
                <p>Our current strategy of single-sourcing from <b>{primary_supplier['Supplier']}</b> is critically exposed. Under the simulated scenario, the average landed cost rises to <b>${baseline_results['Avg Cost']:.2f}</b>, and the probability of a stockout event reaches an untenable <b>{baseline_results['Avg Stockout Risk']:.1f}%</b>.</p>
