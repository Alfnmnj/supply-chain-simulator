# app.py (Strategic Simulation Platform - Final Version)
...
def run_monte_carlo(formula, variables, num_simulations):
    ...
    for var in variables:
        if var['dist'] == 'Normal':
            results[var['name']] = np.random.normal(var['param1'], var['param2'], num_simulations)
        elif var['dist'] == 'Uniform':
            results[var['name']] = np.random.uniform(var['param1'], var['param2'], num_simulations)
        else: # Constant
            results[var['name']] = var['param1']
    
    try:
        final_outcome = pd.eval(formula, local_dict=results)
        return final_outcome
    except Exception as e:
        st.error(f"Error evaluating formula: {e}", icon="🚨")
        return None
...
with st.sidebar:
    ...
    # --- EXPERT MODE UI ---
        ...
        # (This entire section is the robust, bug-fixed version from previous iterations)
...
if run_button:
    ...
    if st.session_state.mode == "Guided":
        # Abstraction Layer: Convert simple inputs to simulation parameters
        ...
        # Scenario 2: Crisis Operations
        crisis_vars = [
            {'name': 'Base_Cost', 'dist': 'Normal', 'param1': base_cost, 'param2': 1.0 + (conc_map[supplier_base] / 100) * 4.0},
            {'name': 'Tariff', 'dist': 'Uniform', 'param1': 0.0, 'param2': (risk_map[geo_risk] / 100) * 0.40}, # Up to 40% tariff
            {'name': 'Supply_Cut_Premium', 'dist': 'Uniform', 'param1': 0.0, 'param2': (risk_map[geo_risk] / 100) * (base_cost * 1.5)} # Premium up to 150% of base cost
        ]
        ...
        if normal_results is not None and crisis_results is not None:
            ...
            stockout_threshold = base_cost * 1.5 # Assume stockout becomes a high risk if cost > 150% of normal
            normal_risk = np.mean(normal_results > stockout_threshold) * 100
            crisis_risk = np.mean(crisis_results > stockout_threshold) * 100
            ...
            # --- Chart Comparison ---
            fig, ax = plt.subplots();
            sns.kdeplot(normal_results, ax=ax, fill=True, label="Normal Operations", color="#1E88E5")
            sns.kdeplot(crisis_results, ax=ax, fill=True, label="Crisis Scenario", color="#D81B60")
            ...
            with st.expander("Show Auto-Generated Business Continuity Plan (BCP)", expanded=True):
                st.markdown(f"""
                <h4>BCP for Critical Component (Avg. Cost: ${base_cost})</h4>
                <p>Based on a scenario with <strong>{geo_risk}</strong> geopolitical risk and a <strong>{supplier_base}</strong> supplier base.</p>
                <h5><i class="bi bi-exclamation-triangle-fill" style="color: #ff4b4b;"></i> Threat Assessment</h5>
                <p>The simulation indicates that in a crisis, the average landed cost per unit could rise from <b>${normal_cost:,.2f}</b> to <b>${crisis_cost:,.2f}</b>. More critically, the probability of a cost-prohibitive stockout event increases from a manageable <b>{normal_risk:.1f}%</b> to an unacceptable <b>{crisis_risk:.1f}%</b>.</p>
                <h5><i class="bi bi-card-checklist"></i> Recommended Actions</h5>
                <ol>
                    <li><b>Immediate (Buffer Stock):</b> Procure an additional 60-90 days of safety stock to mitigate short-term supply shocks. The total estimated risk exposure on an annual volume of {annual_volume:,} units is significant.</li>
                    <li><b>Medium-Term (Supplier Diversification):</b> If operating with a 'Single Source' or 'Consolidated' base, immediately initiate a program to qualify a secondary supplier in a different, low-risk geopolitical region (e.g., domestically in India, or allied nations).</li>
                    <li><b>Long-Term (Design for Resilience):</b> Mandate that new product designs are qualified with at least two different, interchangeable critical components to build resilience into the product lifecycle.</li>
                </ol>
                """, unsafe_allow_html=True)

    # --- EXPERT MODE LOGIC & DISPLAY ---
    else:
        sim_formula = build_formula_from_steps()
        sim_vars = st.session_state.variables
        if not sim_vars or not sim_formula:
            st.warning("Please build your model in the sidebar before running.", icon="⚠️")
        else:
            results = run_monte_carlo(sim_formula, sim_vars, num_simulations)
            if results is not None:
                # (The generic expert dashboard remains the same)
