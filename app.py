...
import pandas as pd
import   Configure your **Resilient Sourcing Mix** with a slider.
    This gives you full control to solve your assignment exactly as the prompt requires.

2.  **Clearer, More Intuitive Visuals:** The dashboard now features a **side-by-side grouped bar chart** powered by Plotly. This is the clearest way to visually compare the `Baseline` vs. `Resilient` strategies across all key metrics (Cost, Lead Time, and Risk) in a single, interactive graph.

3.  **Truly Dynamic & Strategic BCP:** The "Actionable Recommendations" section has been completely rewritten. It is now **100% dynamic**, pulling every number, percentage, and conclusion directly from the live simulation results. It now functions as a true strategic advisor, framing the results in the persuasive language of a manager presenting to their board.

This version is the one we should have arrived at earlier. It is powerful, flexible, user-friendly, and directly tailored to your needs.
...
st.markdown("""
    ...
    <style>
        ...
        .metric-card { background-color: #161b22; border: 1px solid #30363d; padding: 25px; border-radius:  numpy as np
import plotly.express as px
import plotly.graph_objects as go
...
st.markdown("""
    ...
    <style>
        ...
        .metric-card h5 .bi { margin-right: 10px; font-size: 1.510px; text-align: center; }
        .metric-card h5 { margin-bottom: 15px; color: #c9d1d9; font-size: 1.1rem; font-weight: 500; }
...
def load_supplier_data():
    return pd.DataFrame({
        ...
        'Country': ['Taiwan unsafe_allow_html=True)
...
def load_supplier_data():
    return pd.DataFrame({
        ...
        'Base Lead Time (days)': [75, ', 'USA', 'USA', 'Malaysia'],
        'Base Cost ($)': [25.0, 35.0, 150.0, 140.0],
...
if 'variables Risk (%)': [5.0, 2.0, 2.0, 10.0],
        'Is Primary': [True, False, True, False]
    })

supplier_df = load_' not in st.session_state: st.session_state.variables = []
...
if 'model_steps' not in st.session_state: st.session_state.model_steps = []
if 'start_variable' not in st.session_state: st.session_state.start=============
def run_supply_chain_simulation(strategy_suppliers, scenario):
    results = []
    for _, supplier in strategy_suppliers.iterrows():
        tariff = scenario['tariff'] if supplier['Country'] ==_variable = None

# ==============================================================================
# 3. CORE SIMULATION ENGINES
# ==============================================================================
def run_supply_chain_simulation(strategy_suppliers, scenario):
 scenario['country'] else 0
        supply_cut = scenario['supply_cut'] if supplier['Country'] == scenario['country'] else 0
        delay = scenario['delay'] if supplier['Country'] == scenario['    results = []
    for _, supplier in strategy_suppliers.iterrows():
        tariff = scenario['tariff'] if supplier['Country'] == scenario['country'] else 0
        supply_cut = scenario['supply_cut'] if supplier['Country'] == scenario['country'] else 0
        delay = scenario['delay'] if supplier['Countrycountry'] else 0
        ...
        results.append({'Sourcing %': supplier['Sourcing %'], 'Cost': impacted_cost,'] == scenario['country'] else 0
        impacted_cost = supplier['Base Cost ($)'] * (1 + tariff)
        impacted_lead_time = supplier['Base Lead Time (days)'] + delay
        impacted_risk = min(100, supplier['Base Risk (%)'] + (supply_cut * 'Lead Time': impacted_lead_time, 'Stockout Risk': impacted_risk})
    df = pd.DataFrame(results)
    return {'Avg Cost': np.average(df['Cost'], weights=df[' 100))
        results.append({'Sourcing %': supplier['Sourcing %'], 'Cost': impacted_cost, 'Lead Time': impacted_lead_time, 'Stockout Risk': impacted_risk})
    df = pd.DataFrame(results)
    return {'Avg Cost': np.average(df['Cost'],Sourcing %']), 'Avg Lead Time': np.average(df['Lead Time'], weights=df['Sourcing %']), 'Avg Stockout Risk': np.average(df['Stockout Risk'], weights=df['Sourcing %'])}

@st.cache_data
def run_expert_mode_simulation(formula, variables, num_ weights=df['Sourcing %']), 'Avg Lead Time': np.average(df['Lead Time'], weights=df['Sourcing %']), 'Avg Stockout Risk': np.average(df['Stockout Risk'], weights=df['Sourcing %'])}

@st.cache_data
def run_expert_mode_simulation(formula, variables, num_simulations):
    results = {var['name']: (np.random.normal(varsimulations):
    ...
    try: return pd.['param1'], var['param2'], num_simulations) if var['dist'] == 'Normal' else
                             np.random.uniform(var['param1'], var['param2'], num_simulations) if var['dist'] == 'Uniform' else
                             var['param1']) for var in variables}
    try: return pd.eval(formula, local_dict=results)
    except Exception as e: st.error(f"Error: {e}", icon="🚨"); return None

def build_formula_from_steps():eval(formula, local_dict=results)
    except Exception as e: st.error(f"Error: {e}", icon="🚨"); return None
...
#"({formula}) {op_map.get(step['op'])} {step['val'] if step['type'] == 'Variable' else str(step['val'])}"
    return formula
...
with st.sidebar:
    ...
    st.radio("Select Analysis Mode", ["Guided", "Expert"],.imgur.com/vVw2G71.png", width=100)
    st.markdown("<h3><i class='bi bi-tools'></i> Control Panel</h3>", unsafe_allow_html=True)
    st.radio("Select Analysis Mode", ["Guided", "Expert"], key="mode", horizontal=True)
    st.divider()

    if st.session_state.mode == "Guided":
        st.markdown("<h5><i class='bi bi-truck'></i> Supply Chain Risk Simulator</h5>", unsafe_allow key="mode", horizontal=True)
    st.divider()
    ...
    if st.session_state.mode == "Guided":
        ...
        secondary_supplier = component_suppliers[~component_suppliers['Is Primary']].iloc_html=True)
        selected_component = st.selectbox("1. Select Critical Component", supplier_df['Component'].unique())
        component_suppliers = supplier_df[supplier_df['Component'] == selected_component]
        primary_supplier = component_suppliers[component_suppliers['Is Primary']].iloc[0]
        secondary_supplier = component_suppliers[~component_suppliers['Is Primary']].iloc[0]
        st.markdown(f"<p style='font-size: 0.9rem;'>Primary: <b>{primary_supplier['Supplier']}</b> ({primary_supplier['Country']})<br>Alternate: <b>{secondary_[0]
        ...
        with st.expander("2. Define Disruption Scenario", expanded=True):
            disruption_country = st.selectbox("Disrupted Country", supplier_df['Country'].unique(),supplier['Supplier']}</b> ({secondary_supplier['Country']})</p>", unsafe_allow_html=True)
        with st.expander("2. Define Disruption Scenario", expanded=True):
            ...
            disruption_supply_cut = st.slider("Supply Cut Probability index=supplier_df['Country'].unique().tolist().index(primary_supplier['Country']))
            disruption_tariff = st.slider("Tariff Increase", 0, 100, 20, 5, format="%d%%") / 100.0
        ...
        with st.expander", 0, 100, 50, 5, format="%d%%") / 100.0
            disruption_delay = st.slider("Logistics Delay (days)", 0, 60, 14, 1)
...
("3. Configure Resilient Strategy", expanded=True):
            resilient_mix = st.slider(f"Sourcing % from Alternate ({secondary_supplier['Country']})", 0, 100, 40, 5)
        ...
        for i, var in enumerate(st.session_state.variables):
            with st.container():
                ...
                with c1:
                    var    else: # Expert Mode UI
        st.markdown('<h5><i class="bi bi-sliders"></i> 1. Define Variables</h5>', unsafe_allow_html=True)
        for i, var in enumerate(st.session_state.variables):
            with st.container():
                c1,c2 = st.columns([0.85, 0.15])
                with c1:
                    var['name'] = st.text_input("Name", var['name'], key=f"name_{i}").replace(" ", "_"); var['dist'] = st.selectbox("Dist", ["Normal", "Uniform", "Constant"], index=["['name'] = st.text_input("Name", var['name'], key=f"name_{i}").replace(" ", "_"); var['dist'] = st.selectbox("Dist", ["Normal", "Uniform", "Constant"], index=["Normal", "Uniform", "Constant"].index(var['dist']), key=f"dist_{i}")
                    if var['dist'] == "Normal": p1, p2 = st.columns(2); var['param1'] = p1.number_input("Mean (μ)", value=var['param1'], key=f"p1_{i}"); var['param2'] = p2.number_input("StdNormal", "Uniform", "Constant"].index(var['dist']), key=f"dist_{i}")
                    ...
                    elif var['dist'] == "Uniform": p1, p2 = st.columns(2); var['param1'] = p1.number_input("Min", value=var['param1'], key=f Dev (σ)", value=var['param2'], key=f"p2_{i}", min_value=0.0)
                ...
                with c2: 
                    st.write("");"p1_{i}"); var['param2'] = p2.number_input("Max", value=var['param2'], key=f"p2_{i}")
                    else: var['param1'] = st.number_input("Value", value=var['param1'], key=f"p1_{i}"); var['param2'] = 0
                with c2: 
                    st.write(""); st.write("")
        ...
        if st.button("Add Variable", use_container_width=True): st.session_state.variables.append({'name': f'Var_{len(st.session_state.variables)+1}', 'dist': 'Normal', 'paramrerun()
                st.markdown("<hr style='margin:10px 0; border-color: #30363d;'>", unsafe_allow_html=True)
        ...
        var_names = [v['name1': 100.0, 'param2': 10.0}); st.rerun()
        st.divider()
        st.markdown('<h5><i class="bi bi-diagram-3-fill"></i> 2. Build Model</h5>', unsafe_allow_html=True)
...
if run_button:
    if st.session_state.mode == "Guided":
        ...
        scenario = {'country': disruption_country, 'tariff': disruption_tariff, 'supply_'] for v in st.session_state.variables] or [None]
        st.session_state.start_variable = st.selectbox("Start with:", var_names, index=var_names.index(st.session_state.start_variable) if st.session_state.start_variable in var_names else 0)
        for i, step in enumerate(st.session_state.model_steps):
            op_col, type_col, val_col, del_col = st.columns([0.3, 0.25, 0.3, 0.15])
            step['op'] = op_col.selectbox("Then,", ["Add", "Subtract", "Multiply by", "Divide by"], key=f"op_{i}", label_visibility="collapsed")
...
if run_button:
    if st.session_state.mode == "Guided":
        ...
        df_plot = pd.DataFrame([baseline_results, resilient_results], index=['Baseline Strategy', 'Resilient Strategy']).T.reset_index().rename(columns={'index': 'KPI'})cut': disruption_supply_cut, 'delay': disruption_delay}
        with st.spinner("Running comparative simulations..."):
            baseline_results = run_supply_chain_simulation(baseline_suppliers, scenario)
            resilient_results = run_supply_chain_simulation(resilient_suppliers, scenario)
        ...
        with st.expander("Show Actionable Recommendations & BCP", expanded=True):
            ...
            <li><b>Conduct Further Analysis:</b> Use the 'Expert Mode' of this platform to model the financial impact of a line-down event and calculate the full ROI of this resiliencei class="bi bi-shield-check" style="color: #28a745;"></i>  Resilience Investment Analysis (Diversified Strategy)</h5>
            <p>By diversifying <b>{resilient_mix}%</b> of our supply to the alternate fab in <b>{secondary_supplier['Country']}</b>, we achieve a more secure and predictable supply chain:</p>
            <ul><li>The <b>Stockout Risk</b> is reduced by a significant <b>{risk_reduction:.1f} percentage points</b>, down to a more manageable <b>{resilient_results['Avg Stockout Risk']:.1f}%</b>.</li>
            <li>The <b>Landed Cost</b> stabilizes at <b>${resilient_results['Avg Cost']:.2f}</b> per unit. This represents a calculated 'insurance premium' of <b>${cost_premium:.2f}</b> per unit to safeguard against catastrophic production halts.</li>
            <li>The blended <b>Lead Time</b> is projected to be <b>{resilient_results['Avg Lead Time']:.0f} days</b>.</li></ul>
        ...
        if not sim_vars or not sim_formula: st.warning("Please build your model in the sidebar.", icon="⚠️")
            ...
            if results is not None:
                ...
                col1, col2, col3 = st.columns(3, gap="large");
                with col1: st.markdown(f'<div class="metric-card"><h5><i class="bi bi-bullseye"></i>Average Outcome</h5><h2>{mean_val:,.2f}</h2></div> investment.</li></ol>
            """, unsafe_allow_html=True)
        ...
        if not sim_vars or not sim_formula: st.warning("Please build your model in the sidebar.", icon="⚠️")
            ...
            if results is not None:
                ...
                with st.expander("Show Automated Strategic Insights", expanded=True):
                    ...
                    <ul><li><b>Central Tendency:</b> The', unsafe_allow_html=True)
                with col2: st.markdown(f'<div class="metric-card"><h5><i class="bi bi-lightning-charge-fill"></i>Risk (Std. Dev)</h5><h2>{std_val:,.2f}</h2></div>', unsafe_allow_html=True)
                with col3: st.markdown(f'<div class="metric-card"><h5><i class="bi bi-arrows-left-right"></i>90% Confidence Range</h5><h4>{p5:,.2f} — {p95:,.2f}</h4></div>', unsafe_allow_html=True)
                st.divider()
                with st.expander("Show Automated Strategic Insights", expanded=True):
                    ...
                    <li><b>Downside Risk (P5):</b> There is a 5% chance the outcome could be worse than most likely outcome is around <b>{mean_val:,.2f}</b>. Is this aligned with your strategic goals?</li>
                    <li><b>Risk & Volatility:</b> The standard deviation suggests a <b>{risk_text}</b> level of volatility ({risk_level:.1%}). Are your operational plans robust enough to handle this uncertainty?</li>
                    ...
                    <li><b>Upside Potential (P95):</b> Conversely, there is a 5% chance the outcome could be better than <b>{p95:,.2f}</b>. Can your strategy capitalize on this opportunity?</li></ul>""", unsafe_allow_html=True)
                ...
                fig.add_vline(x= <b>{p5:,.2f}</b>. What is your contingency plan for this 'worst-case' scenario?</li>
    ...
    st.markdown("<div style='text-align: center; padding-top: 50px;'><h3 style='color: #8b949e;'>Configure your scenario in themean_val, line_dash="dash", line_color="#FDD835", annotation_text=f"Mean: {mean_val:.2f}")
                fig.update_layout(title_text='Distribution of Potential Outcomes', template='plotly_dark')
                st.plotly_chart(fig, use_container_width=True)
else:
